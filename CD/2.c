...
int comment=0;
char line[100];
int open=0,close=0,openlineno,closelineno; clrscr();
...
lineno++;
getch(); commentcheck=0; comment=0; if(open==1&&close==0)
printf("\n%s",line); if(strstr(line,"/*")&&open==0) {
open=1;close=0;
comment=1; openlineno=lineno; printf("\n%s",line);
} if(strstr(line,"*/")&&close==0&&open==1) {
closelineno=lineno; if(open==1&&close==0) {
close=1; open=0;
